...
public class ControladorDeMira : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        // Converte posição da tela para o centro do canvas
        posicaoCanvas.x = posicaoMouse.x - Screen.width / 2f;
        posicaoCanvas.y = posicaoMouse.y - Screen.height / 2f;

        // Limita o movimento da mira
        posicaoCanvas.x = Mathf.Clamp(
            posicaoCanvas.x,
            -limiteHorizontal,
            limiteHorizontal
        );

        posicaoCanvas.y = Mathf.Clamp(
            posicaoCanvas.y,
            -limiteVertical,
            limiteVertical
        );

        // Suaviza movimento da mira
