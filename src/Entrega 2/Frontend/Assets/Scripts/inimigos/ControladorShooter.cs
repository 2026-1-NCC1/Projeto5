...
public class ControladorShooter : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        // Valores aleatórios para variar comportamento
        faseVertical = Random.Range(0f, Mathf.PI * 2f);

        timerTrocaOrbita =
            Random.Range(tempoMinTrocaOrbita, tempoMaxTrocaOrbita);

        // Guarda rotação inicial do modelo visual
        if (visual != null)
    ...
    private void Update()
    {
        ...
        // Troca direção da órbita aleatoriamente
    ...
    private void FixedUpdate()
    {
        ...
        // Direção lateral da órbita
        Vector3 dirDireitaOrbita =
            Vector3.Cross(Vector3.up, dirFrente).normalized;

        Vector3 componenteOrbita =
            dirDireitaOrbita * (direcaoOrbita * velocidadeOrbita);

        // Controle de distância do jogador
        float erro = distancia - distanciaIdeal;

        float componenteAproxima = 0f;

        if (erro > toleranciaDistancia)
            componenteAproxima = 1f;
        else if (erro < -toleranciaDistancia)
            componenteAproxima = -1f;

        Vector3 componenteDistancia =
            dirFrente * (componenteAproxima * (velocidadeMaxima * 0.65f));

        // Movimento vertical oscilando
        faseVertical += Time.fixedDeltaTime * frequenciaVertical;

        Vector3 componenteVertical =
            Vector3.up * (Mathf.Sin(faseVertical) * amplitudeVertical);
        ...
        velocidadeAlvo =
            Vector3.ClampMagnitude(velocidadeAlvo, velocidadeMaxima);
        ...
        Quaternion rotAlvo =
            Quaternion.LookRotation(dirFrente, Vector3.up);
    ...
    private void AplicarInclinacaoVisual()
    {
        ...
        // Calcula inclinação lateral da nave
        float velLateral =
        ...
        Quaternion rotAlvo =
            rotacaoVisualInicial * Quaternion.Euler(0f, 0f, roll);
    ...
    private void Atirar()
    {
        ...
        Transform origem =
            pontoDeDisparo != null ? pontoDeDisparo : transform;
        ...
        // Inicializa o movimento do projétil
    ...
    {
        // Aplica dano ao jogador em colisão
